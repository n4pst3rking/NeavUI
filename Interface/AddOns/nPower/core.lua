...
local f = CreateFrame('Frame', 'nPowerFrame', UIParent)
f:SetScale(1.4)
SetSize(f, 18, 18)
...
SetSize(f.Power, config.sizeWidth, 3)
...
f.Power.Below:SetVertexColor(0.5, 0.5, 0.5)
...
f.Power.Above:SetVertexColor(0.5, 0.5, 0.5)
...
local function GetRealMpFive()
  local _, activeRegen = GetManaRegen()
...
local function UpdateBarVisibility()
  ...
  elseif (not InCombatLockdown() and UnitMana('player') > 0) then
...
local function UpdateArrow()
    ...
    local newPosition = UnitMana('player') / UnitManaMax('player') * f.Power:GetWidth()
    f.Power.Below:SetPoint('TOP', f.Power, 'BOTTOMLEFT', newPosition, 0)
end

local function UpdateBarValue()
    local min = UnitMana('player')
    f.Power:SetMinMaxValues(0, UnitManaMax('player', f))
...
subscribe(f, 'OnEvent', function(self, event, arg1)
